...
- **毒舌 AI 对话** — 以 `.` 开头触发，流式返回，逐句实时发送到群
- **表情包自动收集** — 群内图片自动通过视觉模型分析情绪，归类保存
...
1. **Paper 1.21+** 服务端
2. **OneBot 协议端**（如 NapCat / Lagrange）运行在 `ws://127.0.0.1:3001`
3. **火山方舟** 豆包 API 账号
...
3. 确保以下模型已开通：
   - `doubao-seed-2-0-lite-260215`（对话，大模型）
   - `doubao-seed-2-0-mini-260215`（对话，小模型）
   - `doubao-1-5-vision-pro-32k`（图片分析，视觉模型）
...
VISION-MODEL: "doubao-1-5-vision-pro-32k"
GROUP: "你的群号"
SELF-QQ: "机器人QQ号"
OP-QQ: "管理员QQ号"
```
...
3. 预存表情包（可选）：将 `src/main/resources/` 下的 4 张 JPG 按情绪放入对应子目录：
...
群内任何人发的图片都会自动被视觉模型分析。如果判定为表情包且情绪匹配 7 类之一，自动下载保存到 `emojis/对应情绪/` 目录，供 AI 后续随机使用。
